...
export const Editar = async (bodys: any,file:any,user:any) => {
    ...
    let { id,nome, descricao, preco, tipo,afiliacao,porcentagemAfialiacao,idAreaMembro } = body
    let resposta: any
    let idProdutor = user?.idConta
    if(id && nome && tipo  && idProdutor ){
        let valida = await Valida(nome,idProdutor,id)
        if(valida){
            let editar = await Produto.findOne({ where: { idProdutor,id } })
            if (editar?.id) {
                Object.assign(editar, {nome, descricao, preco, tipo,afiliacao:afiliacao=='true'?true:false,porcentagemAfialiacao:porcentagemAfialiacao?porcentagemAfialiacao:0,...(idAreaMembro && {idAreaMembro})})
