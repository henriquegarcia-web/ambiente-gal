...
export default function ClientsPage() {
    ...
    const columns = [
        ...
        { field: "email", headerName: "Email", minWidth: 500, align: "center" },
        { field: "cpf", headerName: "CPF", minWidth: 600, align: "center" },
        {
    ...
    const handleListagem = async () => {
        ...
        if (response.list?.length > 0) {
            let listagem = response.list?.map((item) => { return { id: item.id, nome: item.nome, cpf: item.cpfCnpj, email: item.email, cep: item.cep, rua: item.rua, numero: item.numero, bairro: item.bairro, cidade: item.cidade, estado: item.estado } })
    ...
    return (
        <div className="flex items-start justify-start">
            ...
            <ClientsForm disabled={disabled} buttonText={buttonText} listagem={listagem} setListagem={setListagem} modalActive={modalActive} setModalActive={setModalActive} item={item} setItem={setItem} />
        </div>
    );
}
