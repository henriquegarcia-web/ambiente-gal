...
const AreaMembroForm = ({ buttonText, memberModalActive, setMemberModalActive, item, disabled, setListagem, setItem, listagem }) => {
        ...
        onSubmit: async (values)=>{
            ...
            if (values.id) {
                ...
                if (response.error) {
                    ...
                    Object.assign(list[index], { nome:values.nome, descricao:values.descricao, ...(urlDaImagem && { imagem: urlDaImagem }) })
                    setListagem(list)
                ...
                if (response.error) {
                    ...
                    alert(response.msg)
                    setMemberModalActive(false)
                    let value = { id: response.id, nome:values.nome, descricao:values.descricao,link:response.link, ...(urlDaImagem && { imagem: urlDaImagem }) }
                    let list = [...listagem]
                    list.push(value)
                    setListagem(list)
                    validation.resetForm()
    ...
    return (
        <>
            {memberModalActive && (
                <Modal
                    modalActive={memberModalActive}
                    handleClickModal={() => setMemberModalActive(false)}
                    titleModal={buttonText}
                >
                    {['Excluir Área'].includes(buttonText) &&
                        <>
                            <div className="row" >
                    ...
                    {['Cadastrar Área', 'Editar Área', 'Visualizar Área'].includes(buttonText) &&
