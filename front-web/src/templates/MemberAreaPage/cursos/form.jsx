...
const CursosForm = ({ buttonText, courseModalActive, setCourseModalActive, item, disabled, setListagem, setItem, listagem, areaMembros }) => {
        ...
        onSubmit: async (values) => {
            ...
            if (values.id) {
                ...
                if (response.error) {
                    ...
                    Object.assign(list[index], { idAreaMembro: values.idAreaMembro, areaMembro: areaMembros.find((item) => item.id == parseInt(values.idAreaMembro)), nome: values.nome, descricao: values.descricao, ...(urlDaImagem && { imagem: urlDaImagem }) })
                    setListagem(list)
                ...
                if (response.error) {
                    ...
                    alert(response.msg)
                    setCourseModalActive(false)
                    let value = { id: response.id, idAreaMembro: values.idAreaMembro, areaMembro: areaMembros.find((item) => item.id == parseInt(values.idAreaMembro)), nome: values.nome, descricao: values.descricao, ...(urlDaImagem && { imagem: urlDaImagem }) }
                    let list = [...listagem]
                    list.push(value)
                    setListagem(list)
                    validation.resetForm()
    ...
    return (
        <>
            ...
            {courseModalActive && (
                <Modal
                    modalActive={courseModalActive}
                    handleClickModal={() => setCourseModalActive(false)}
                    titleModal={buttonText}
                >
                    {['Excluir Curso'].includes(buttonText) &&
                        <>
                            <div className="row" >
                    ...
                    {['Cadastrar Curso', 'Editar Curso', 'Visualizar Curso'].includes(buttonText) &&
                        ...
                        >
                            <div className="flex items-center justify-start relative my-5">
                                <select type="text" value={validation?.values?.idAreaMembro} onChange={validation.handleChange} name="idAreaMembro" disabled={disabled} id="idAreaMembro"
                                    ...
                                    <option value="" >Área de Membro</option>
