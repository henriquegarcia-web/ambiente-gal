...
export default function UserPage() {
    ...
    const columns = [
        ...
        { field: "email", headerName: "Email", minWidth: 300, align: "center" },
       
        { field: "permissao", headerName: "Permissão", minWidth: 250, align: "center",renderCell: (params) => (
            <div style={{ textAlign: 'center', display: 'flex' }}>
                {params.row.permissao==1?'Admin':params.row.permissao==4?"Gerente":params.row.permissao==5?"Funcionario":''}
            </div>
        ) },
    ...
    const handleListagem = async () => {
        ...
        let listagem = response.list?.map((item) => { return {...item,status: (item.cpfCnpj && item.cep && item.rua && item.numero && item.bairro && item.cidade && item.estado && item.banco && item.titular && item.tipoConta && item.conta && item.agencia)?"Completo":"Incompleto"  } })
    ...
    const handleFilter = (e) => {
        ...
        let list = listagem.filter((item) => item.nome.toLocaleLowerCase().includes(buscar.toLocaleLowerCase()) || item.email.toLocaleLowerCase().includes(buscar.toLocaleLowerCase())  || (item.permissao==1?'Admin':item.permissao==4?"Gerente":"Funcionario").toLocaleLowerCase().includes(buscar.toLocaleLowerCase()))
    ...
    return (
        <div className="flex items-start justify-start">
            ...
            <UserForm disabled={disabled} buttonText={buttonText} listagem={listagem} setListagem={setListagem} modalActive={modalActive} setModalActive={setModalActive} item={item} setItem={setItem} />
        </div>
    );
}
